...
public class GameStatus : MonoBehaviour
{
    ...
    //Highway minigame variables
    private int carHitCount;
    private bool playerPassedHighway;
    //Sockgame sock boolean
    private bool sockGameWon;
    ...
    //GAMEWORLD:
    //Misc. variables related to the game world.
    private bool gameStartedFirstTime;
    ...
    void Awake()
    {
        ...
        playerPoints = 0; //Players "currency" is set to 0 by default.
        ...
        //Highway minigame variables
        carHitCount = 0;
        playerPassedHighway = true; //true by default, but when they lose it is set to false. It was just simpler to do it quickly.
        //Sockgame sock boolean
        sockGameWon = false;
    ...
    void Update()
    {
        ...
        if(GetMinutes() <= 0 && GetSeconds() <= 0 && timerOn)
        {
            SetTimerOn(false);
            SetEnteredChangeAreaStatus(true);
            Vector3 outdoorPosition = new Vector3(82.0f, 0.0f, 0.0f);
            SetPlayerPosition(outdoorPosition);
            SceneManager.LoadScene("Piha");
        }
        if(currentScene.name == "Piha")
        {
            SetTimerOn(false);
        }
    ...
    //Highwayrun minigame methods
    public int GetCarHitCount()
    {
        return carHitCount;
    }
    public void SetCarHitCount(int setter)
    {
        carHitCount = setter;
    }
    public void AddCarHitCount()
    {
        carHitCount += 1;
    }
    public bool GetPlayerPassedHighway()
    {
        return playerPassedHighway;
    }
    public void SetPlayerPassedHighway(bool setter)
    {
        playerPassedHighway = setter;
    }

    //SockGameWon methods
    public bool GetSockGameWon()
    {
        return sockGameWon;
    }
    public void SetSockGameWon(bool setter)
    {
        sockGameWon = setter;
    }
    ...
    //GAMEWORLD:
    //Misc. variables related to the game world.
    public bool GetGameStartedFirstTime()
    {
        return gameStartedFirstTime;
    }
    public void SetGameStartedFirstTime(bool setter)
    {
        gameStartedFirstTime = setter;
    }
}
